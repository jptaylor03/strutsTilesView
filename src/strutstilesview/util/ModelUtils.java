...
import strutstilesview.util.MiscUtils.ResourceType;
import strutstilesview.util.classloading.MyLoadedClass;
...
public class ModelUtils {
	 ...
	 */
	private static Class resourceBaseClass = StrutsTilesView.class;
	 ...
	 */
	public static SimpleTreeNode createBranchViewerAbout(SimpleTreeNode parent, ViewerAppInfo appInfo) {
		SimpleTreeNode va = new SimpleTreeNode("viewer-about", parent);
		va.add(new SimpleTreeNode("applicationName="         +appInfo.getAppName(), parent));
		va.add(new SimpleTreeNode("applicationVersion="  +"v"+appInfo.getAppVersion(), parent));
		va.add(new SimpleTreeNode("applicationBuildDate="    +appInfo.getAppBuildDate(), parent));
		va.add(new SimpleTreeNode("applicationAuthorContact="+appInfo.getAppAuthorContact(), parent));
		...
		for (int x = 0; x < domChildren.getLength(); x++) {
			...
			if (domNodeType == Node.ELEMENT_NODE) {
				...
				if ("target-app-config-file-masks".equals(domNodeName)) {
					...
					vc.setTargetAppConfigFilesByMask(MiscUtils.createFileRelatedMapFromList(vc.getTargetAppConfigFileBasePath(), vc.getTargetAppConfigFileMasks(), ResourceType.FILES));
				} else
				...
				if ("target-app-source-code-path-masks".equals(domNodeName)) {
					...
					vc.setTargetAppSourceCodePathsByMask(MiscUtils.createFileRelatedMapFromList(vc.getTargetAppSourceCodeBasePath(), vc.getTargetAppSourceCodePathMasks(), ResourceType.DIRECTORIES));
				} else
				...
				if ("target-app-object-code-path-masks".equals(domNodeName)) {
					...
					vc.setTargetAppObjectCodePathsByMask(MiscUtils.createFileRelatedMapFromList(vc.getTargetAppObjectCodeBasePath(), vc.getTargetAppObjectCodePathMasks(), ResourceType.DIRECTORIES));
				} else
				...
				if ("target-app-classpath-path-masks".equals(domNodeName)) {
					...
					vc.setTargetAppClasspathPathsByMask(MiscUtils.createFileRelatedMapFromList(vc.getTargetAppClasspathBasePath(), vc.getTargetAppClasspathPathMasks(), ResourceType.FILES));
				} else
