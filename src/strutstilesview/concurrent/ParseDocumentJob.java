package strutstilesview.concurrent;
		...
		try {
			for (int x = 0; x < mapNames.length; x++) {
				...
				if ("viewer-config".equals(tagName)) {
					...
					work.put("viewer-about", ModelUtils.createBranchViewerAbout(parent, StrutsTilesView.appInfo));
					...
					StrutsTilesView.viewerConfig = (ViewerConfig)((SimpleTreeNode)work.get("viewer-config")).getUserObject();
				} else {
